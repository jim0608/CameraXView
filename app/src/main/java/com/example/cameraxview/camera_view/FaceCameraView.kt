...
import com.example.cameraxview.FaceRectHelper
import com.example.cameraxview.R
import com.mysafe.msmealorder_customer.activitys.camera.camera_view.CameraImageAnalyze
import com.mysafe.msmealorder_customer.activitys.camera.camera_view.FaceCameraHelper
import com.mysafe.msmealorder_customer.activitys.camera.camera_view.FaceRectView
...
//        val isDisplayPortrait = measuredWidth>measuredHeight
     ...
     * 是否根据预览画面大小剪切
     */
    fun isClipPicture(isClip: Boolean = false) {
        this.isClip = isClip
    }
     ...
     */
    fun setMirror(isXMirror: Boolean = false, isYMirror: Boolean = false) {
...
//        mIconImageView.setImageResource(R.drawable.ic_baseline_photo_camera)
        mPreviewView.previewStreamState.observe(lifecycleOwner) {
            when (it) {
                StreamState.IDLE -> {
                    Log.i(TAG, "setPreviewStreamState: StreamState.IDLE")
                ...
                StreamState.STREAMING -> {
                    Log.i(TAG, "setPreviewStreamState: StreamState.STREAMING")
                    isStreaming = true
                    removeViewAt(2)
